...
class RepairsController extends AdminController
{
    ...
    public function index(){
        ...
        $list = M('Repairs')->where($map)->order('id asc')->select();
    ...
    public function add(){
        if(IS_POST){
            $Repairs = D('Repairs');
            $info = M('Repairs')->order('id desc')->limit(1)->find();
            $data = $Repairs->create();
            $Repairs->serial_number=date('Ymd').sprintf("%06d", $info['id']+1);
            if($data){
                $id = $Repairs->add();
                if($id){
    ...
    public function del(){
        ...
        if(M('Repairs')->where($map)->delete()){
            //记录行为
            action_log('update_Repairs', 'channel', $id, UID);
            $this->success('删除成功');
        } else {
            $this->error('删除失败！');
        }
